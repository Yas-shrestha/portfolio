<x-app-layout>
            ...
            <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
                <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                    ...
                    <div class="flex items-center gap-3 px-6 py-4 border-b border-gray-100">
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-800">Edit project</p>
                            <p class="text-xs text-gray-400">Update the information below</p>
                        </div>
                    </div>
                        ...
                        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; padding: 16px;">
                            @foreach ($files as $file)
                                <div @click="toggleImage({{ $file->id }}, '{{ asset('storage/' . $file->file_name) }}')"
                                    style="position: relative; cursor: pointer; height: 100px; overflow: hidden; border-radius: 8px;">
    ...
    {{-- Pre-load existing project images into Alpine --}}
    <script>
        function projectForm() {
            return {
                ...
                selectedImages: @json(
                    $project->images->map(fn($img) => [
                            'id' => $img->file_id,
                            'url' => asset('storage/' . $img->file->file_name),
                        ])),
